...
export function generateOutreachMessages(lead) {
  ...
  // Pre-baked per-lead messages (generated by scripts/lead-message-gen) take
  // priority over the category templates. Fall back per-platform when the
  // pre-baked version is missing.
  const baked = lead.messages || {};

  const messages = {};

  messages.instagramDm = {
    message: baked.instagram || templates.instagramDm(l),
  };

  messages.tiktokDm = {
    message: baked.tiktok || templates.tiktokDm(l),
  };

  if (baked.email && baked.email.subject && baked.email.body) {
    messages.email = { subject: baked.email.subject, body: baked.email.body };
  } else {
  ...
  messages.linkedin = {
    message: baked.linkedin || templates.linkedin(l),
  };

  // Text and phone aren't in the baked set (0-2 leads have phone, no text
  // channel in the data), so always use the category templates.
